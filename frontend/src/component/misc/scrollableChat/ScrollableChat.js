          ...
          messages.map((m) => {
            return m.sender._id !== user._id ? (
              <div className=" w-full flex mt-1">
                <div className="max-w-72  bg-slate-900 text-slate-100 shadow-md p-1 rounded-sm  text-xs   break-words">
                  {m.content}
                </div>
              </div>
            ) : (
              <div className=" w-full mt-1 flex flex-row-reverse">
                <motion.div
                  className="max-w-72 text-right shadow-md p-1 text-xs rounded-sm break-words"
